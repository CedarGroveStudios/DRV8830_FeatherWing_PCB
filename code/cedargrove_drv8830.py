# SPDX-FileCopyrightText: 2022 Cedar Grove Studios
...
class DRV8830:
    ...
    def throttle_volts(self, new_throttle_volts):
        ...
        if new_throttle_volts < 0:
            self._vset = VoltageAdapter._voltage_to_index(self, abs(new_throttle_volts))
            self._in_x = BridgeControl.REVERSE
        elif new_throttle_volts > 0:
            self._vset = VoltageAdapter._voltage_to_index(self, new_throttle_volts)
            self._in_x = BridgeControl.FORWARD
        else:
            self._vset = 0
            self._in_x = BridgeControl.BRAKE
